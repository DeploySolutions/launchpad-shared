...
namespace Deploy.LaunchPad.Geospatial
{
    public partial class GeospatialHelper : HelperBase
    {
        ...
        public virtual bool ValidateWgs84Point(Point point, bool shouldThrowIfInvalid = false)
        {
            ...
            if (lat < -90)
            {
                ...
                if (shouldThrowIfInvalid)
                    throw new ArgumentOutOfRangeException(Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Set_Latitude_Not_LessThan_Minus_90);
            }
            else if (lat > 90)
            {
                isValid = false;
                if (shouldThrowIfInvalid)
                    throw new ArgumentOutOfRangeException(Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Set_Latitude_Not_GreaterThan_90);
            }
            else if (lon < -180)
            {
                isValid = false;
                if (shouldThrowIfInvalid)
                    throw new ArgumentOutOfRangeException(Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Set_Longitude_Not_LessThan_Minus180);
            }
            else if (lon > 180)
            {
                isValid = false;
                if (shouldThrowIfInvalid)
                    throw new ArgumentOutOfRangeException(Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Set_Longitude_Not_GreaterThan_180);
            }
            if (!isValid && shouldThrowIfInvalid)
                throw new ArgumentException(Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Validate_Point);
            return isValid;
        }
        ...
        public virtual GeographicPositionDto GetGeographicPositionDto(string geoJson, double? elevation, Coordinate? userDefinedCenter = null, double[]? userDefinedBoundingBox = null)
        {
            ...
            Guard.Against<ArgumentException>(double.IsNaN(elevation.Value), Deploy_LaunchPad_Geospatial_Resources.Guard_GeographicLocation_Set_Elevation);
