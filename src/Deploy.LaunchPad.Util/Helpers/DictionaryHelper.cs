...
namespace Deploy.LaunchPad.Util
{
    ...
    /// Implements the <see cref="Deploy.LaunchPad.Util.HelperBase" />
    /// </summary>
    /// <seealso cref="Deploy.LaunchPad.Util.HelperBase" />
    public partial class DictionaryHelper : HelperBase
    {
        ...
        public virtual IDictionary<TKey, TValue> AddToDictionary<TKey, TValue>(IDictionary<TKey, TValue> dictionary, TKey key, TValue item, bool shouldOverwriteIfExists = false, DictionaryHelperLoggingStrategy loggingStrategy = DictionaryHelperLoggingStrategy.LogOnlyOverwrites)
        {
            ...
            if (!dictionary.ContainsKey(key))
            {
                ...
                {
                    Logger.Debug(string.Format(Deploy_LaunchPad_Util_Resources.Logger_Info_ItemAdded, item));
                }                
            }
            else
            {
                if (shouldOverwriteIfExists)
                {
                    ...
                    {
                        Logger.Debug(string.Format(Deploy_LaunchPad_Util_Resources.Logger_Info_ItemAlreadyExistsOverwriting, item));
                    }                    
                }
                else
                {
                    ...
                    {
                        Logger.Debug(string.Format(Deploy_LaunchPad_Util_Resources.Logger_Info_ItemAlreadyExists, item));
                    }
                }
            }
            return dictionary;
